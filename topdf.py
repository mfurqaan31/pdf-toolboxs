...
def save_uploaded_images(uploaded_images):
    ...
    for i, uploaded_image in enumerate(uploaded_images, start=1):
        # Get the original filename and change the extension to .png
        ...
        # Save the image with the new filename and .png extension
...
def display_images(uploaded_paths, reordered_paths):
    ...
    for i in range(num_rows):
        ...
        # Display three images horizontally in a row
        row_images_display = []
        ...
        # Display the images with three images per row
...
def main():
    ...
    if uploaded_images:
        ...
        # Display the images for rearranging
        st.write("Arrange the order of the images")
        reordered_paths = sort_items([os.path.basename(path) for path in uploaded_paths])  # Use sort_items with filenames
        ...
        if st.button("Convert to PDF"):
            # Append ".pdf" extension if not provided by the user
